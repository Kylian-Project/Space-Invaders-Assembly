...
I_buff: .word 0 # Adresse de l'allocation mÃ©moire de image
I_buff_end: .word 0 # Adresse de fin de l'allocation mÃ©moire de image
...
main:
	# Charger la valeur 0x00ff0000 pour couleur rouge
    	...
    	# Charger l'adresse 0x10010000 dans t1 qui est l'adresse de depart
	#li t1 0x10010000
	...
	jal I_creer # Allouer la mémoire
...
I_xy_to_addr:
	lw a2 I_buff # adresse de dÃ©part de mÃ©moire "image"
...
I_addr_to_xy:
	...
	sub a0 a0 t1 # Soustraction à adresse de depart pour récuperer la difference
	loop2:
...
I_creer:
	...
	srli s6 s6 4 # Diviser le nombe de bits pour récup l'adresse de fin après
	
	li a7 9 # instruction d'allocution mÃ©moire en bits
	ecall
	
	sw a0 (a1) # Stocker le contenu de a0 Ã  l'adresse de I_buff	
...
I_buff_to_visu:
	...
	loop_transfer:
        	lw t3 0(t1)  # Charger la donnée depuis I_buff
        	sw t3 0(t0)  # Stocker la donnée dans I_visu
        	addi t0 t0 4  # Avancer dans I_visu
        	addi t1 t1 4  # Avancer dans I_buff

		beq t1 t2 end_loop_transfer  # Répéter tant que nous n'avons pas atteint la fin de I_buff
		j loop_transfer
